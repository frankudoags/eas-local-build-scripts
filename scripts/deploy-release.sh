...
echo -e "${BLUE}🚀 Release Deployment Script${NC}\n"
...
echo -e "${BLUE}📱 Building and submitting release builds...${NC}\n"

# Build Android
if [ "$PLATFORM" = "android" ] || [ "$PLATFORM" = "all" ]; then
  echo -e "${BLUE}🤖 Building Android Release...${NC}"
  eas build --platform android --profile release --local --non-interactive --output=./android-release.aab

  echo -e "${BLUE}📤 Submitting to Google Play Console...${NC}"
  eas submit -p android --path ./android-release.aab --profile release --non-interactive
...
if [ "$PLATFORM" = "ios" ] || [ "$PLATFORM" = "all" ]; then
  ...
  eas build --platform ios --profile release --local --non-interactive --output=./ios-release.ipa

  echo -e "${BLUE}📤 Submitting to App Store Connect...${NC}"
  eas submit -p ios --path ./ios-release.ipa --profile release --non-interactive

  echo -e "${GREEN}✅ iOS release build completed!${NC}\n"
fi

echo -e "\n${GREEN}🎉 Release deployment completed!${NC}"

# Step 3: Create GitHub release if requested
if [ "$CREATE_RELEASE" = true ]; then
  echo -e "\n${BLUE}🏷️  Creating GitHub release...${NC}"
  "$SCRIPT_DIR/create-release.sh" --type release --non-interactive
  echo -e "${GREEN}✅ GitHub release created!${NC}"
else
  echo -e "${YELLOW}💡 To create a GitHub release, run:${NC}"
  echo -e "${BLUE}   ./scripts/create-release.sh --type release${NC}"
fi